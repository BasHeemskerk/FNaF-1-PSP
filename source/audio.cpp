...
namespace call{
    OSL_SOUND* phoneCalls[5];
    bool stopped = false;

    void loadPhoneCalls(){
        if (save::whichNight == 1){
            phoneCalls[0] = oslLoadSoundFileWAV("romfs/ambience/office/call/call1.wav", OSL_FMT_STREAM);
        }
        else if (save::whichNight == 2){
            phoneCalls[1] = oslLoadSoundFileWAV("romfs/ambience/office/call/call2.wav", OSL_FMT_STREAM);
        }
        else if (save::whichNight == 3){
            phoneCalls[2] = oslLoadSoundFileWAV("romfs/ambience/office/call/call3.wav", OSL_FMT_STREAM);
        }
        else if (save::whichNight == 4){
            phoneCalls[3] = oslLoadSoundFileWAV("romfs/ambience/office/call/call4.wav", OSL_FMT_STREAM);
        }
        else if (save::whichNight == 5){
            phoneCalls[4] = oslLoadSoundFileWAV("romfs/ambience/office/call/call5.wav", OSL_FMT_STREAM);
        }
    }
    void playPhoneCalls(){
        if (save::whichNight == 1){
            oslPlaySound(phoneCalls[0], 1);
        }
        else if (save::whichNight == 2){
            oslPlaySound(phoneCalls[1], 1);
        }
        else if (save::whichNight == 3){
            oslPlaySound(phoneCalls[2], 1);
        }
        else if (save::whichNight == 4){
            oslPlaySound(phoneCalls[3], 1);
        }
        else if (save::whichNight == 5){
            oslPlaySound(phoneCalls[4], 1);
        }

        if (oslGetSoundChannel(phoneCalls[0]) == -1){
            unloadPhoneCalls();
        }
        else if (oslGetSoundChannel(phoneCalls[1]) == -1){
            unloadPhoneCalls();
        }
        else if (oslGetSoundChannel(phoneCalls[2]) == -1){
            unloadPhoneCalls();
        }
        else if (oslGetSoundChannel(phoneCalls[3]) == -1){
            unloadPhoneCalls();
        }
        else if (oslGetSoundChannel(phoneCalls[4]) == -1){
            unloadPhoneCalls();
        }
    }
    void unloadPhoneCalls(){
        if (save::whichNight == 1){
            oslDeleteSound(phoneCalls[0]);
        }
        else if (save::whichNight == 2){
            oslDeleteSound(phoneCalls[1]);
        }
        else if (save::whichNight == 3){
            oslDeleteSound(phoneCalls[2]);
        }
        else if (save::whichNight == 4){
            oslDeleteSound(phoneCalls[3]);
        }
        else if (save::whichNight == 5){
            oslDeleteSound(phoneCalls[4]);
        }
        stopped = true;
    }
}
